...
gulp.task('plugins', () => {
    return Promise.all([
        ...
        { name: 'RevealCustomControls', input: './plugin/customcontrols/plugin.js', output: './plugin/customcontrols/customcontrols' },
    ].map( plugin => {
        return rollup({
...
gulp.task('css-core', () => gulp.src(['css/custom-styles.scss'])
    .pipe(compileSass())
    .pipe(autoprefixer())
    .pipe(minify({compatibility: 'ie9'}))
    .pipe(header(cssLicense))
